            ...
            <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                ...
                <div class="relative bg-gray-900 rounded-lg border border-gray-200 text-white shadow dark:bg-gray-700">
                    ...
                    <div class="p-6 space-y-6">
                        <form id="frmFlights" name="frmFlights" class="w-full" action="" method="POST"
                            @submit.prevent>
                            @csrf
                            <div class="mb-4">
                                <select name="airline_id" id="airline_id" :class="selectBoxClass"
                                    @change="onChangeAirline(flight.airline_id)" v-model="flight.airline_id">
                                    <option value="0">Select Airline</option>
                                    <option v-for="airline in airlines" :value="airline.id">@{{ airline.name }}
                                    </option>
                                </select>
                                <span class="text-red-500" id="airline_id_error"></span>
                                <select :disabled="flight.airline_id == 0" name="origin_id" id="origin_id"
                                    :class="selectBoxClass" @change="onChangeOrigin(flight.origin_id)"
                                    v-model="flight.origin_id">
                                    <option value="0">Select Origin</option>
                                    <option v-for="city in airline.cities" :value="city.id">@{{ city.name }}
                                    </option>
                                </select>
                                <span class="text-red-500" id="origin_id_error"></span>
                                <select :disabled="flight.origin_id == 0" name="destination_id" id="destination_id"
                                    :class="selectBoxClass" v-model="flight.destination_id">
                                    <option value="0">Select Destination</option>
                                    <option v-for="city in airlineCitiesWithoutOrigin" :value="city.id">
                                        @{{ city.name }}</option>
                                </select>
                                <span class="text-red-500" id="destination_id_error"></span>
                                {{-- Departure Date --}}
                                <div class="relative">
                                    <label for="departure_date" class="text-sm font-medium text-gray-400">Departure
                                        Date</label>
                                    <input type="date" name="departure_date" id="departure_date" :class="selectBoxClass"
                                        style="color-scheme: dark" v-model="flight.departure_date">
                                    <span class="text-red-500" id="departure_date_error"></span>
                                </div>
                                {{-- Arrival Date --}}
                                <div class="relative">
                                    <label for="arrival_date" class="text-sm font-medium text-gray-400">
                                        Arrival Date
                                    </label>
                                    <input type="date" name="arrival_date" id="arrival_date"
                                        :class="datesAreInvalid ?
                                            'form-input block w-full mt-1 bg-gray-900 text-white border-2 border-red-500 rounded-lg' :
                                            'form-input block w-full mt-1 bg-gray-900 text-white border-2 border-white rounded-lg'"
                                        style="color-scheme: dark" v-model="flight.arrival_date">
                                    <span class="text-red-500" id="arrival_date_error">
                                        @{{ datesAreInvalid ? "Psst... Unless you can travel threw time, you can't arrive before your departure" : "" }}
                                    </span>
                                </div>
                            </div>
                            <div>
                                <button id="btn-create" :disabled="!readyForSubmit"
                                    @click="isEdit ? updateFlight() : createFlight()"
                                    :class="readyForSubmit ? 'bg-blue-500 text-white px-4 py-3 rounded font-medium w-full' :
                                        'bg-blue-200 text-white px-4 py-3 rounded font-medium w-full'">
                                    Save Changes
                                </button>
                            </div>
                        </form>
                ...
                <div class="px-6 py-4">
                    ...
                    <div>
                        <div class="mb-4 flex inline-flex">
                            ...
                            <select id="selectCity" @change="onChangeCity(city.id)"
                                class="ml-4 bg-gray-800 text-white border-2 border-white rounded-lg max-h-12 overflow-scroll"
                                v-model="city.id">
                ...
                <div id="pagination" class="px-6 py-4 text-center">
                    <div class="flex flex-col items-center">
                        ...
                        <span class="text-sm text-gray-200 dark:text-gray-400">
                            Showing <span class="font-semibold text-white dark:text-white">@{{ flights.from }}</span>
                            to <span class="font-semibold text-white dark:text-white">@{{ flights.to }}</span> of
                            <span class="font-semibold text-white dark:text-white">@{{ flights.total }}</span> Entries
                        </span>
                        <nav aria-label="Page navigation example">
                            <ul class="inline-flex -space-x-px mt-3">
                                <li v-for="(page, index) in flights.links" class="page-item">
                                    <button @click="changePage(page)" :disabled="page.active"
                                        :hidden="index == 0 || index == flights.last_page + 1"
                                        :class="page.active ? activePageClass : defaultPageClass"
                                        :data-url="page.url">@{{ page.label }}</button>
                                </li>
                            </ul>
                        </nav>
                    </div>
